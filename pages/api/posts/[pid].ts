...
blogPosts.set('my-blog-post', {
  title: 'My Blog Post',
  content: `Ullamco adipisicing nostrud esse ex fugiat magna consequat. Commodo anim excepteur irure proident proident est culpa quis aliquip. Culpa in eu occaecat pariatur elit sit ea nulla mollit sit proident ipsum aute dolore. Ipsum cupidatat occaecat dolor ad Lorem officia nisi. Aliqua in esse nulla veniam in qui nisi qui duis in anim aute adipisicing nulla. Officia est nulla qui aliqua.
...
blogPosts.set('another-blog-post', {
...
export default function handle(req, res) {
  const blogPost = blogPosts.get(req.query.pid);

  if (!blogPost) return res.status(404).json({ message: 'Not found' });
  res.json({ ...blogPost });
}
